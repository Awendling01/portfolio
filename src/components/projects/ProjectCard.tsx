...
// Project content in `content.ts` is normalized to 6 highlights and 4 metrics
// per project. Each section below uses an explicit `lg:min-h-*` so cards
// render at uniform internal placement when sitting side-by-side, regardless
// of which card's content is longest in any given section.
  ...
  const inner = (
    <Card hover={!!detailHref} className="h-full flex flex-col">
      ...
      <div className="flex items-start justify-between gap-4">
        <h3 className="flex-1 min-w-0 text-xl sm:text-2xl font-bold text-white tracking-tight leading-tight lg:min-h-[3.75rem]">
          <span
            className={
              detailHref
      ...
      {/* Meta */}
      <div className="mono mt-5 text-[11px] uppercase tracking-[0.18em] text-[var(--text)] leading-snug lg:min-h-[2.25rem]">
        {project.role}
      ...
      {/* Description */}
      <p className="mt-5 text-[var(--text2)] text-[15px] leading-relaxed lg:min-h-[9.75rem]">
        {project.description}
      </p>
      ...
      {showHighlights ? (
        <ul className="mt-5 flex-1 grid sm:grid-cols-2 gap-x-6 gap-y-2.5 content-start">
          {project.highlights.map((h) => (
            <li
              key={h}
              className="relative pl-5 text-sm text-[var(--text2)] leading-relaxed lg:min-h-[4.5em]"
            >
